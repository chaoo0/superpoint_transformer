...
def main():
    # --- 1. 配置文件与路径设置 ---
    ckpt_path = "spt-2_s3dis_fold5.ckpt"  # 您的权重文件

    # 定义三大文件夹路径
    RAW_DIR = "data/real/raw"  # 存放原始 .ply
    NAG_DIR = "data/real/nag"  # [新增] 存放计算好的超点图 .pt 文件
    HTML_DIR = "data/real/html"  # [新增] 存放最终的 HTML 可视化网页
    ...
    print(f">>> 找到 {len(ply_files)} 个 .ply 文件，开始批量处理 (NAG存储 + 分割 + HTML导出)。")
    ...
    for i, ply_path in enumerate(ply_files):
        ...
        nag_device.show(
            ...
            max_points=100000,
            path=html_path,
            title=f"SPT 分割结果 - {base_name}"  # 网页标题
        )
